# Kokoro TTS Flutter (Summify vendored copy)

> **Summify:** это вендоренная копия с патчем загрузки `voices.json` с диска для скачанных моделей. См. [`FOR_SUMMIFY_MAINTAINERS.md`](FOR_SUMMIFY_MAINTAINERS.md). Корневой обзор папки `packages/`: [`../README.md`](../README.md).
