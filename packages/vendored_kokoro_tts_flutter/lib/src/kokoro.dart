...
import 'kokoro_voices_read_from_disk_io.dart'
    if (dart.library.html) 'kokoro_voices_read_from_disk_stub.dart'
    as kokoro_voices_disk;
...
class Kokoro {
  ...
  Future<void> _loadVoices() async {
    try {
      // Asset keys (e.g. assets/voices.json) must use rootBundle; downloaded
      // models live on disk and must be read with File — rootBundle would throw
      // "Unable to load asset" for absolute paths.
      final String jsonString;
      if (_kokoroVoicesPathIsFlutterBundleKey(config.voicesPath)) {
        jsonString = await rootBundle.loadString(config.voicesPath);
      } else {
        jsonString = await kokoro_voices_disk
            .loadKokoroVoicesJsonFromFilesystemPath(config.voicesPath);
      }
  ...
  /// [KokoroConfig.voicesPath] is either a Flutter asset key or an on-disk path.
  static bool _kokoroVoicesPathIsFlutterBundleKey(String path) {
