...
### 8.1 Ограничение на длину текста

**Важно**: Kokoro TTS имеет ограничение на максимальное количество фонем в одном запросе - **510 фонем**.

- Это ограничение исходит из архитектуры модели ONNX
- Не документировано в официальной документации библиотеки
- Обнаружено эмпирически из ошибки: `Exception: Text is too long, must be less than 510 phonemes`

**Решение**:
- Автоматическая обрезка текста до 510 фонем перед синтезом
- Проигрывание только первой части длинного текста
- Уведомление пользователя о том, что текст был обрезан

**Пример обработки**:
```dart
// Проверка количества фонем
final phonemeCount = phonemes.split(RegExp(r'\s+'))
    .where((p) => p.isNotEmpty)
    .length;

if (phonemeCount > 510) {
  // Обрезка до 510 фонем
  final truncatedPhonemes = phonemeTokens.take(510).join(' ');
...
5. **Ограничение длины** - учитывайте лимит в 510 фонем на запрос
