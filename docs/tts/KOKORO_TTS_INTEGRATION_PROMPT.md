...
## 📁 Быстрый старт: Какие файлы копировать?

**См. подробный список:** [FILES_TO_COPY.md](./FILES_TO_COPY.md)

**Минимальный набор:**
1. `assets/tokenizer_vocab.json` - словарь токенизации (ОБЯЗАТЕЛЬНО)
2. `lib/patches/kokoro_tts_flutter_patched/` - патченная библиотека (ОБЯЗАТЕЛЬНО)
3. `scripts/generate_tokenizer_vocab.py` - скрипт генерации словаря (РЕКОМЕНДУЕТСЯ)

**Где находятся токены?**
- `assets/tokenizer_vocab.json` - это и есть словарь токенизации (177 токенов)

## ⚠️ КРИТИЧЕСКИ ВАЖНО: Установка пакета

**НЕ используйте:**
- ❌ `kokoro_tts_flutter: ^0.2.0` (из pub.dev - не подходит)
- ❌ `git: url: https://github.com/...` (несуществующий репозиторий)

**Используйте:**
- ✅ `path: lib/patches/kokoro_tts_flutter_patched` (локальная папка)

**Порядок действий:**
1. Скопируйте папку `lib/patches/kokoro_tts_flutter_patched/` в ваш проект
2. Добавьте в `pubspec.yaml`: `kokoro_tts_flutter: path: lib/patches/kokoro_tts_flutter_patched`
...
flutter:
  assets:
    ...
    - assets/us_gold.json      # ОБЯЗАТЕЛЬНО для фонетизации английского
    - assets/us_silver.json    # ОБЯЗАТЕЛЬНО для фонетизации английского
...
## 3. Копирование патченной библиотеки (ОБЯЗАТЕЛЬНО)

**КРИТИЧЕСКИ ВАЖНО:** Не используйте оригинальный пакет `kokoro_tts_flutter` из pub.dev!

### 3.1 Скопируйте патченную библиотеку

Скопируйте папку `lib/patches/kokoro_tts_flutter_patched/` из этого проекта в ваш проект:
...
**Важно:**
- Эта версия содержит исправления для правильной токенизации
- Содержит логирование для отладки
- Используйте именно эту версию, а не оригинальную из pub.dev

### 3.2 Зависимости (pubspec.yaml)
...
dependencies:
  ...
  # Kokoro TTS - используйте ЛОКАЛЬНЫЙ ПУТЬ к патченной версии
  kokoro_tts_flutter:
    path: lib/patches/kokoro_tts_flutter_patched
  # НЕ используйте:
  #   git: url: https://github.com/... (это не работает!)
  #   version: ^0.2.0 (оригинальная версия не подходит)
...
1. Убедитесь, что папка `lib/patches/kokoro_tts_flutter_patched/` скопирована полностью
2. Проверьте, что путь в `pubspec.yaml` правильный: `path: lib/patches/kokoro_tts_flutter_patched`
3. Убедитесь, что в папке есть файл `pubspec.yaml`
...
**Готовый скрипт:** В проекте уже есть папка `test_kokoro/` с готовым тестовым скриптом.
...
Скрипт выведет токены для фразы "Because I fell in love." в формате, который можно сравнить с логами Flutter приложения.

**Или создайте свой тестовый скрипт:**
...
1. Запустите `test_kokoro/test_kokoro_tokens.py` и скопируйте токены
