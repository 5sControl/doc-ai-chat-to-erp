...
#### Подарочный баланс (gift balance):
- Пользователь может ввести **подарочный код** в настройках (Settings → «Ввести подарочный код»). Валидный код один раз добавляет **+10 документов** в подарочный баланс.
- **Приоритет при создании:** сначала используется дневной бесплатный слот (1 в день), затем при исчерпанном дне — списание с подарочного баланса (по 1 за документ). Когда и день исчерпан, и подарочный баланс 0 — показывается paywall, создание не разрешается (или создание помечается как заблокированное при share intent).
- Один код можно активировать только один раз на устройстве/аккаунте (список использованных кодов хранится в `redeemedGiftCodes`).
- Валидные коды задаются в коде приложения (`validGiftCodes`); при необходимости проверку можно перенести на backend.

#### Ограничения:
- Нет доступа к премиум функциям
- Ограниченное количество summary (1 в день + подарочный баланс при наличии кода)
...
4. **Промо-коды**: Доступны через StoreKit, но требуют настройки на стороне App Store Connect. См. [App Store: бесплатный период и Offer Codes](./app_store_free_period_offer_codes.md).
