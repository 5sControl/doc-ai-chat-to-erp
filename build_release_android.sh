...
# Скрипт для автоматического инкремента build number и сборки релизного AAB для Android

set -e  # Остановить при ошибке

echo "🚀 Начинаем процесс сборки релиза Android..."
...
# Собираем релизный AAB
echo "🔨 Собираем релизный AAB..."
flutter build appbundle --release

if [ $? -eq 0 ]; then
    AAB_PATH="build/app/outputs/bundle/release/app-release.aab"
    AAB_SIZE=$(du -h "$AAB_PATH" | cut -f1)
    echo -e "${GREEN}✅ Релизный AAB успешно собран!${NC}"
    echo "📦 Файл: $AAB_PATH"
    echo "📊 Размер: $AAB_SIZE"
